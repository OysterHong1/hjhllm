...
function hasAttachmentUrl(value) {
  return (
    ...
    (value.startsWith("http://") ||
      ...
      value.startsWith("/api/attachments/files/"))
  );
}
...
async function cleanupSmokeData() {
  ...
  await api("/api/admin/reset-demo-data", {
    method: "POST",
...
assert(
  attachmentMessage.attachments.every((attachment) =>
    hasAttachmentUrl(attachment.url)
  ),
  "Attachment upload did not return an attachment URL"
);
...
assert(
  hasAttachmentUrl(audioMessage.attachments[0].url),
  "Audio upload did not return an attachment URL"
);
...
assert(
  hasAttachmentUrl(videoMessage.attachments[0].url),
  "Video upload did not return an attachment URL"
);
  ...
  beforeReplyMessages.some(
    (message) =>
      ...
      message.attachments.every(
        (attachment) =>
          attachment.mimeType === "image/png" && hasAttachmentUrl(attachment.url)
      )
  ),
...
assert(
  beforeReplyMessages.some(
    (message) =>
      ...
      hasAttachmentUrl(message.attachments[0].url)
  ),
...
assert(
  userMessages.some(
    (message) =>
      ...
      message.attachments.length === 2 &&
      message.attachments.every(
        (attachment) => attachment.storagePath && hasAttachmentUrl(attachment.url)
      )
  ),
